              ...
              : ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      title: Text(
                        'Date: ${vente.date.substring(0, 16).replaceFirst('T', ' ')}',
                        style: GoogleFonts.poppins(fontWeight: FontWeight.w600),
                      ),
                      subtitle: Text(
                        'Total Vente: ${Formatters.formatCurrency(vente.total)} DA',
                      ),
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return FutureBuilder<List<Map<String, dynamic>>>(
                              future: DatabaseHelper().database.then(
                                (db) => db.query(
                                  'articles',
                                  where: 'id = ?',
                                  whereArgs: [va.articleId],
                                ),
                              ),
                              builder: (context, snapshot) {
                                ...
                                return ListTile(
                                  title: Text(name),
                                  subtitle: Text(
                                    'Quantité vendue: ${va.quantity}',
                                  ),
                                  trailing: Text(
                                    '${Formatters.formatCurrency(va.quantity * va.price)} DA',
                                  ),
                                );
                              },
                            );
